...
import type { ChartDataPoint, Order, PortfolioSummary, SystemHealth } from "../../types";
...
const DashboardPage: React.FC = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      if (history.status === "fulfilled") {
        setPortfolioHistory((history.value as { timestamp: string; total_value: number }[]).map((item) => ({
          x: item.timestamp,
          y: item.total_value
        })));
      }
  ...
  const chartData = {
    labels: portfolioHistory.map((h) => new Date(h.x as string).toLocaleDateString()),
    datasets: [
      {
        label: "Portfolio Value",
        data: portfolioHistory.map((h) => h.y),
        borderColor: "#3B82F6",
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          ...
          value={
            portfolioSummary?.total_value
              ? `$${Number(portfolioSummary.total_value).toLocaleString()}`
              : "Loading..."
          }
          change={portfolioSummary?.daily_pnl_percent ?? 0}
        ...
        <StatCard
          title="Today's P&L"
          value={
            portfolioSummary?.daily_pnl !== undefined
              ? `$${Number(portfolioSummary.daily_pnl).toLocaleString()}`
              : "Loading..."
          }
          change={portfolioSummary?.daily_pnl_percent ?? 0}
          icon={
            portfolioSummary && portfolioSummary.daily_pnl >= 0
              ? ArrowTrendingUpIcon
              : ArrowTrendingDownIcon
          }
          color={portfolioSummary && portfolioSummary.daily_pnl >= 0 ? "green" : "red"}
      ...
      {systemHealth && (
        <div className="bg-white shadow-sm rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-900">
                {systemHealth.last_heartbeat
                  ? new Date(systemHealth.last_heartbeat).toLocaleTimeString()
                  : "N/A"}
              </div>
